...
const { friendDetails, changeStatus} = props
  return (
    <div className='friend-friends container'>
      ...
      <div className='friend-info'>
        <div >
          ...
          <p>Married: {friendDetails.married ? 'Yes':'No'} <button onClick={() => {changeStatus(friendDetails.id)}}>change</button></p>
          <div>Likes:
            <ul>
              {/* 👉 3- Loop over the friend's hobbies and generate <li/> elements as you go */}
              {friendDetails.hobbies.map(hobby => {
